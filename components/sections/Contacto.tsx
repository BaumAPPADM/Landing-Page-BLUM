...
export default function Contacto(p: any) {
  ...
  return (
    <section id="contacto" style={s("position:relative;padding:110px 5%;overflow:hidden;background-image:url(\"" + (urlFor(d?.contactoImage) || "/assets/foto-dos-personas-campo.jpg") + "\");background-size:cover;background-position:75% 50%;")}>
          ...
          {(formSent) ? (
            <>
              <p style={s("font-family:'IBM Plex Sans',sans-serif;font-size:15px;line-height:1.6;color:#0d6b64;font-weight:600;margin:0;text-align:center;padding:40px 0;")}>
                {d?.formSuccessMessage || "Gracias — nuestro equipo te contactará en menos de 48 horas."}
              </p>
            </>
          ) : null}
          {" "}
